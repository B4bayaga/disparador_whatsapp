...
def test_repr(session):
    ...
    """
    mensagem = connection.Mensagens(cliente="Rafael", telefone=123456789, enviado=True)
    session.add(mensagem)
    session.commit()

    assert repr(mensagem) == "Cliente: Rafael, Telefone: 123456789, Enviado: True, Data: {}".format(datetime.now().date())
...
def test_insert(session):
    ...
    """
    mensagem = connection.Mensagens(cliente="Rafa Oliveira", telefone=77999548524, enviado=True)
    session.add(mensagem)
    session.commit()
...
def test_insert_session(session):
    ...
    """
    mensagem = connection.Mensagens(cliente="Pax Nacional", telefone=7794247117, enviado=True)
    session.add(mensagem)
    session.commit()
